...
export class BaseRouter {
  ...
  constructor(table: h.Helpers.SqlTableType, disableGetAll: boolean) {
    ...
    debug('is this really working?'+ this.table.getFieldNames(true));
      ...
      (async function query() {
        try {
          if (req.body.id) {
            ...
            getResult  = await this.table.getById(req.body.id)
            if (getResult != null) {
              throw new Error('an existing item already exists');
            }
          }
          let insResult = await this.table.insert(req.body);
