...
import acTable = require('./dal/activitiesTable');
import acRouter = require('./routers/activitiesRouter');
import todosTable = require('./dal/todosTable');
...
class main {
  public run(): void {
    ...
    const pool: sql.ConnectionPool = new sql.ConnectionPool(config, (err) => {
      ...
      const activitiesTable: acTable.ActivitiesTable = new acTable.ActivitiesTable(pool);
      const activitiesRouter: acRouter.ActivitiesRouter = new acRouter.ActivitiesRouter(activitiesTable, false);
      app.use('/activities', activitiesRouter.router)

      const tt: todosTable.TodosTable = new todosTable.TodosTable(pool);
      const todosRouter: baseRouter.BaseRouter = new baseRouter.BaseRouter(tt, false);
