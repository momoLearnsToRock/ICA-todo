...
export class SqlTableType extends h.Helpers.TableType {
  ...
  constructor({ connectionPool, tableName, viewName, fields, autoGeneratedPrimaryKey, throwOnExtraFields }: { connectionPool: sql.ConnectionPool, tableName: string, viewName: string, fields: h.Helpers.SqlField[], autoGeneratedPrimaryKey: boolean, throwOnExtraFields: boolean }) {
  ...
  parseFieldsInJsonBody({ includeId, jsonBody, throwOnMissingFields, throwOnMissingModifiedOn, throwOnExtraFields, sqlReq, queryFields }:
    ...
    if (throwOnExtraFields) {
      ...
      bodyKeys.forEach((item, index) => {
        if (queryFields.map((f: h.Helpers.SqlField) => { return f.name }).indexOf(item) < 0) {
